# Paper Assistant 项目搭建与面试解析文档

## 1. 项目定位

Paper Assistant 可以包装成一个“垂直行业知识服务平台”的底层模块：面向科研论文、岗位 JD、行业报告、旅游攻略、美妆成分资料等非结构化内容，把长期积累的资料转成可检索、可解释、可复用的知识资产。当前代码以科研论文为样例场景，核心是把 PDF 论文解析成结构化文本，再交给 LightRAG 建立向量索引与知识图谱，最后支持问答、跨文档对比、知识图谱可视化和 RAG 效果评估。

简历定位建议：

> 基于 LightRAG + Ollama + Docling 构建垂直行业知识服务平台，以科研论文为首个落地场景，实现 PDF 语义解析、GraphRAG 混合检索、知识图谱可视化、MCP 工具化暴露和 RAG 评估闭环，并针对长文档处理中的异步超时与显存竞争问题完成工程化排查和稳定性优化。

这个项目更适合体现 AI 应用工程师的知识工程能力：文档解析、索引构建、检索增强生成、评估指标、工具协议暴露、工程排障。公司层面可以服务职业规划、旅游、美妆、教育、法律等“高信息差 + 强知识依赖”的咨询场景。
...
- 商业化连接：Paper Assistant 沉淀行业知识，Second Agent 沉淀用户画像和任务流程，组合后可以做垂直行业顾问 Agent。
...
```text
docs/MCP_AND_SKILL_EXTENSION_PLAN.md # MCP/Skill 扩展设计与简历表达
...
- 将系统抽象为垂直行业知识服务层，可迁移到职业规划、旅游规划、美妆护肤等高信息差咨询场景，为上层 Agent 提供可信知识调用能力。
